			...
			$.ajax({
				...
				success:function(remoteData){
					if(remoteData.codeId==1){
						...
						if(_this.remoteDataArr.length<1){
							$el.find("#sure_sel").text("暂无门店")
							document.getElementById("sure_sel").disabled=true;
						}else{
							$el.find("#sure_sel").text("确认")
							document.getElementById("sure_sel").disabled=false;
						}
						$("#left").html(content);
