                              ...
                              'on-change':(event) => {
                                this.parchseData[params.index].car_sku_listdata.forEach(function(item){
                                    if(item.id == event){
                                      ...
                                      setTimeout(function () {
                                         _this.getAddressPrice(item.id,params.index)
                                      },300)
                                      setTimeout(function () {
        ...
        this.$api.get('/dealer/getOne',data,reset=>{
          if (reset.codeId===1) {
             var data = reset.resData
              _this.shopinfo.principalsName  = data.contactName
              _this.shopinfo.principalsTelephone  = data.contactTelephone
              _this.shopinfo.address  = data.provinceName + data.cityName+data.areaName+data.address
              _this.shopinfo.provinceCode  = data.provinceCode
              _this.shopinfo.cityCode  = data.cityCode
              _this.getcar()
          }
        })
      },
      ...
      getAddressPrice(e,index){
        ...
        this.$api.get('/car_purchase_price/list',data,reset=>{
          if (reset.codeId===1) {
            ...
            _this.parchseData[index].purchase_quantity = 0
          } else {
      ...
      getshopOne(e){
        ...
        this.$api.get('/dealer_storefront/getOne',data,reset=>{
          ...
          if (reset.codeId === 1) {
            ...
            var str = ""
            if (_this.shopinfo.provinceName) {
              str+=_this.shopinfo.provinceName
            }
            if (_this.shopinfo.cityName) {
              _str+=_this.shopinfo.cityName
            }
            if (_this.shopinfo.address) {
              str+=_this.shopinfo.address
            }
