<template>
   <div class="SideMenuComponent">
        <div class="mermber-logo"><i></i>经销商管理后台</div>
                  ...
                  <div class="">
                    <Submenu :name="index" v-for="(item,index) in items" v-if="item.isShow!=-1">
                        <template slot="title">
                          ...
                          <MenuItem :name="index+'-'+itemlistindex" v-for="(itemlist,itemlistindex) in item.dealerResourcesDoList">
                             {{itemlist.name}}
                          </MenuItem>
                    </Submenu>
               </div>
...
export default {
  ...
  beforeCreate () {
    ...
    this.$api.get('/dealer_role/getByToken',data,reset => {
        if (reset.codeId === 1) {
  ...
  methods: {
    ...
    changeMenu (e) {
      ...
      this.$router.push({path:this.items[muneindex].dealerResourcesDoList[itemindex].path})
