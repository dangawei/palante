<template>
  <div class="">
    <div id="J-container" :style="montorstyle"></div>
    <index-user-component :Emitsidedata="Emitsidedata" v-if="usershow" @emitcomponent="emitcomponent" :bottom="400"></index-user-component>
    <div class="sideSystem" v-if="show">
        <span @click="open" class="montor-dic" :class="{montorbottm:!montorshow}">{{openshow}}</span>
        <div class="montorBottom" v-if="montorshow">
            <div class="montor-hd">
                <div class="montor-tab">
                  <ul>
                    <li @click="tabs(1)" :class="{ative:montorshow01}">实时在线</li>
                    <li @click="tabs(2)" :class="{ative:montorshow02}">历史轨迹</li>
                  </ul>
                </div>
            </div>
            <div class="montor-bd">
              <div class="tabmontorBox">
                   <car-component :dynamicdata="Emitsidedata" v-if="montorshow01" @carclick="carclick"></car-component>
                   <history-component :vin="Emitsidedata.vin" :settime="settime" v-if="montorshow02"></history-component>
              </div>
              <div class="tabBottom">
                  <div class="">VIN码:{{Emitsidedata.vin}}</div>
                  <div class="">SIM:{{Emitsidedata.simNo}}</div>
                  <div class="">心跳时间：{{dynamicdata.lastHeart | alltime}}</div>
                  <div class="">最后定位时间：{{dynamicdata.lastLocationTime | alltime}}</div>
                  <div class="">经纬度:{{dynamicdata.longitude}},{{dynamicdata.latitude}}</div>
                  <div class="">实时位置：{{dynamicdata.address}}</div>
              </div>
            </div>
            <div class="positionselect" v-if="montorshow01">
              <Select v-model="openLock" @on-change="openLocklist" placeholder="车辆控制"  style="margin-right:15px">
